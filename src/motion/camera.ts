...
export type SceneMeta = {
  ...
  /**
   * Optional cap from the asset's data-max-fov. A scene without overscan
   * cannot be framed at full canvas width without exposing a band edge on the
   * first frame of parallax, and only the asset knows that about itself.
   */
  maxFovUnits?: number;
};
 ...
 */
export function maxFov(aspect: number, canvasH: number, declared?: number): number {
  const geometric = aspect * canvasH;
  return declared === undefined ? geometric : Math.min(geometric, declared);
}
...
export function frameFor(beat: CameraBeat, aspect: number, scene: SceneMeta): Frame {
  const capW = maxFov(aspect, scene.canvas.h, scene.maxFovUnits);
  let w: number;
